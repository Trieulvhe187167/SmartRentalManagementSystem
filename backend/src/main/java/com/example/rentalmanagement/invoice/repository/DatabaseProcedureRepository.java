...
public interface DatabaseProcedureRepository extends JpaRepository<Invoice, Long> {
    @Modifying
    @Query(value = "CALL sp_recalculate_invoice_payment(:invoiceId)", nativeQuery = true)
    public void recalculateInvoicePayment(@Param("invoiceId") Long invoiceId);

    @Modifying
    @Query(value = "CALL sp_mark_overdue_invoices()", nativeQuery = true)
