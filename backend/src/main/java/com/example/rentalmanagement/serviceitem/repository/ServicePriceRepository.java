package com.example.rentalmanagement.serviceitem.repository;
...
public interface ServicePriceRepository extends JpaRepository<ServicePrice, Long> {
    @Query("""
            ...
            where p.isDeleted = false
              and p.serviceItem.id = :serviceId
              and p.effectiveFrom <= :date
              and (p.effectiveTo is null or p.effectiveTo >= :date)
            order by p.effectiveFrom desc
            """)
    public List<ServicePrice> findEffective(@Param("serviceId") Long serviceId, @Param("date") LocalDate date);

    @Query("""
            select count(p) > 0 from ServicePrice p
            where p.isDeleted = false
              and p.serviceItem.id = :serviceId
              and p.effectiveFrom <= :to
              and (p.effectiveTo is null or p.effectiveTo >= :from)
            """)
    public boolean overlaps(@Param("serviceId") Long serviceId, @Param("from") LocalDate from, @Param("to") LocalDate to);

    public Page<ServicePrice> findByServiceItemIdAndIsDeletedFalse(Long serviceId, Pageable pageable);
}
